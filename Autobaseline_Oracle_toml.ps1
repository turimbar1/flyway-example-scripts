...
flyway-dev init -n $projectName -p $workingFolderPath --database-type $databaseType --i-agree-to-the-eula

# Read the JSON file
$projectPath = Join-Path $workingFolderPath "flyway.toml"
flyway-dev project-update -p=$workingFolderPath -u=true --developmentDatabase.connectionProvider.url=$Url --developmentDatabase.connectionProvider.user=$User --developmentDatabase.connectionProvider.password=$Password --developmentDatabase.connectionProvider.schemas=$s --i-agree-to-the-eula
...
$diffOptions | ConvertTo-Json | flyway-dev diff -p $workingFolderPath -a $artifactPath --from Target --to Empty --output json --i-agree-to-the-eula | flyway-dev apply -p $workingFolderPath -a $artifactPath --verbose --i-agree-to-the-eula 
